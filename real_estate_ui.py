...
# File Paths
# -----------------------------
lr_model_path = r"C:\Infosys\real_estate_pipeline.pkl"
prophet_models_path = r"C:\Infosys\all_regions_prophet_models.pkl"
    ...
    if os.path.exists(prophet_models_path):
        ...
        try:
            ...
            if isinstance(loaded_models, dict):
                ...
                st.warning(f"⚠️ Prophet file is not a dict (got {type(loaded_models)}).")
        except Exception as e:
            st.error(f"❌ Error loading Prophet models: {e}")
    else:
        st.warning("⚠️ Prophet models file not found.")
...
if st.button("Predict Price"):
    if not lr_model:
        ...
        try:
            ...
            if len(parts) != 9:
                st.error("⚠️ Please enter exactly 9 comma-separated values.")
            else:
...
if regions:
    ...
    st.warning("⚠️ Prophet models could not be loaded. Forecasting unavailable.")
